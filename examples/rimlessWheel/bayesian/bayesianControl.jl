...
@model function fitLoss(X0, μ_prior::AbstractArray{T}, σ_prior::AbstractArray{T}; totalTimeStep = 2000) where {T<:Real}
    ...
    # 0.0 ~ Normal(l/batch_size, LogExpFunctions.softplus.(s[1]))
    0.0 ~ Distributions.Normal(l/length(X0), 20.0f0)
end
...
function controlToHipSpeed()
    ...
    param = Float32.(vcat(randn(paramNum), invsoftplus.(0.3f0*rand(paramNum))))
    μ_param, σ_param = unstackParams(param)
    ...
    for i in 1:500
        ...
        step > 10 ? popfirst!(elbo_prev) : nothing
        push!(elbo_prev, vo(alg, getq(param), model, elbo_num))

        ##########################################################
        converged = hasconverged(X0[1], param, elbo_prev) 

        ProgressMeter.next!(prog)
    end
end

function hasconverged(x0, param, elbo_prev)

    filtered_elbo = filter_elbo(elbo_prev)

    if mod(step, 10) == 0.0 
        println("elbo = ", round(elbo_prev[end], digits=4))

        w         = rand(getq(param))
        X         = trajectory(x0, [], w; totalTimeStep=5000)
        loss      = hipSpeedLoss(X)
        plots(X, fig1)
        subplot(2, 2, 4)
        plot(filtered_elbo[end], marker=".", color="k") 
